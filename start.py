...
def create_toefl_materials(tpo_neo, number):
    ...
    filenames = [
        os.path.join(text_dir, f"{tpo_neo.upper()}{number}_Listening-C1.md"),
        os.path.join(text_dir, f"{tpo_neo.upper()}{number}_Listening-L1.md"),
        os.path.join(text_dir, f"{tpo_neo.upper()}{number}_Listening-L2.md"),
        os.path.join(text_dir, f"{tpo_neo.upper()}{number}_Listening-C2.md"),
        os.path.join(text_dir, f"{tpo_neo.upper()}{number}_Listening-L3.md"),
        os.path.join(text_dir, f"{tpo_neo.upper()}{number}_Listening-L4.md"),
    ...
    if tpo_neo.upper() == 'TPO':
        ...
        query = f"SELECT listening_record_recordScript FROM tpo_listening_record WHERE listening_record_recordId LIKE '{tpo_neo.upper()}_{number}_L%' ORDER BY listening_record_recordId"
        cursor.execute(query)
        rows = cursor.fetchall()
